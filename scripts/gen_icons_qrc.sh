...
if [[ ! -d ${lingmoui_dir}/${BREEZEICONS_DIR} ]]; then
	echo "could not find ${BREEZEICONS_DIR}, please clone ocean-icons first into ${BREEZEICONS_DIR}:"
	echo "cd ${lingmoui_dir} && git clone --depth 1 https://invent.kde.org/frameworks/ocean-icons.git ${BREEZEICONS_DIR}"
	exit 1
fi
